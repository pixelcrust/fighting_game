...
@onready var animation_player: AnimationPlayer = $Base_Rig_004.animation_player
...
func turn_left(delta):
	print(str(rad_to_deg($Base_Rig_004.rotation.y)))
	$Base_Rig_004.rotation.y = deg_to_rad(180.0)
	is_facing_right = false

func turn_right(delta):
	print(str(rad_to_deg($Base_Rig_004.rotation.y)))
	$Base_Rig_004.rotation.y = deg_to_rad(0.0)
	is_facing_right = true

func _on_base_rig_004_being_hit_to_main_script(dmg: float, stagger: float) -> void:
